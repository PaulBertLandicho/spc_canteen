...
$sql = "SELECT product.*, cart.quantity, cart.order_date FROM product INNER JOIN cart ON product.id = cart.product_id WHERE cart.user_id = $user_id";
...
<div><br><br>
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<span class="product-total-price" style="font-size:20px;">₱' . $row['price'] . '</span>'; // Total price
            // Create a form for updating quantity
            $product_price = $row['price'] * $row['quantity'];
            echo '<div class="product">';
            echo '<span class="quantity" style="font-size: 10px;">x' . $row['quantity'] . '</span>'; // Quantity
            echo '</div>';
            echo '<span class="order-date" style="font-size: 14px; color: maroon; margin-right:1px;">' . $row['order_date'] . '</span><br>';
            // Add cancel button with link to cancel the order
            echo '<a href="cancel.php?cancel_product_id=' . $row['id'] . '" class="btn btn-danger" style="background-color:white; border-color:maroon; color:maroon;">Cancel</a>';
            // Add delete button with link to delete the product
            echo '<a href="Order.php?cancel_order_id=' . $row['id'] . '" class="btn btn-danger" style="margin-left: 15px; background-color:maroon;">See Order</a>';
            echo '</div>';
            echo '</div>';
        }
    } else {
