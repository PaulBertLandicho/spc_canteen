...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($_FILES['image']['size'] > 0) {
        ...
        // Update product with the new image and Available status
        $sql = "UPDATE product SET name='$name', price='$price', category='$category', time_to_cook='$time_to_cook', image='$target_file', available='$available' WHERE id=$product_id";
    } else {
        // Update product without changing the image but update ready status
        $sql = "UPDATE product SET name='$name', price='$price', category='$category', time_to_cook='$time_to_cook', available='$available' WHERE id=$product_id";
    }
